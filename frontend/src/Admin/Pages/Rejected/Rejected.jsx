...
const Rejected = () => {
            ...
            renderCell: (params) => {
                return (
                    <Box sx={{ display: 'flex', gap: 3 }}>
                        <Button
                            variant='outlined'
                            onClick={() => acceptCollege(params.row._id)}
                        >
                            Accept
                        </Button>
    ...
    const fetchCollege = () => {
        axios.get('http://localhost:5000/CollegeRejected').then((response) => {
            console.log(response.data.Collegelist)
            setRows(response.data.Collegelist)
        })
    }

    const acceptCollege = (Id) => {
        axios.put(`http://localhost:5000/acceptCollege/${Id}`).then((response) => {
